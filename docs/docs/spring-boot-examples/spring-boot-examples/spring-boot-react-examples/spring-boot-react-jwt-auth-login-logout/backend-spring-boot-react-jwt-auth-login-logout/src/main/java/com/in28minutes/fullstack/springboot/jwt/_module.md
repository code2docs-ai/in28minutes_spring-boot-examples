...
| 名称 | jwt |
| 编码语言 | .java |
| 代码路径 | spring-boot-examples/spring-boot-react-examples/spring-boot-react-jwt-auth-login-logout/backend-spring-boot-react-jwt-auth-login-logout/src/main/java/com/in28minutes/fullstack/springboot/jwt |
| 包名 | spring-boot-examples.spring-boot-react-examples.spring-boot-react-jwt-auth-login-logout.backend-spring-boot-react-jwt-auth-login-logout.src.main.java.com.in28minutes.fullstack.springboot.jwt |
...
该流程图展示了Spring Boot项目中JWT认证模块的层级结构。从`jwt`目录开始，逐步展开到`basic`、`authentication`、`springbootjwtauthloginlogout`等子目录，并进一步细分为`jwt`和`course`两个子模块。每个子模块中包含多个Java类文件，如`JwtTokenResponse.java`、`CourseResource.java`等，展示了项目的文件组织和依赖关系。

# 文件列表 File List

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [basic](basic/_module.md) | package | 基于Spring Boot和React的JWT认证系统，支持登录、注销及令牌生成验证，确保系统安全性。 |
