...
| 名称 | basic |
| 编码语言 | .java |
| 代码路径 | spring-boot-examples/spring-boot-react-examples/spring-boot-react-jwt-auth-login-logout/backend-spring-boot-react-jwt-auth-login-logout/src/main/java/com/in28minutes/fullstack/springboot/jwt/basic |
| 包名 | spring-boot-examples.spring-boot-react-examples.spring-boot-react-jwt-auth-login-logout.backend-spring-boot-react-jwt-auth-login-logout.src.main.java.com.in28minutes.fullstack.springboot.jwt.basic |
...
graph TD
    ...
    basic --> SpringBootFullStackJwtAuthLoginLogoutApplication.java
...
该流程图展示了Spring Boot项目中基于JWT的认证和课程管理模块的层级结构。`basic`作为根节点，包含`authentication`和主应用类`SpringBootFullStackJwtAuthLoginLogoutApplication.java`。`authentication`下分为`springbootjwtauthloginlogout`，进一步细分为`jwt`和`course`两个子模块。`jwt`模块包含多个与JWT相关的类和配置，而`course`模块则包含课程相关的资源和服务类。

# 文件列表 File List

| 名称   | 类型  | 说明 |
|-------|------|-------------|
| [authentication](authentication/_module.md) | package | 基于Spring Boot和React的JWT认证系统，支持登录、注销及令牌生成验证，确保系统安全性。 |
